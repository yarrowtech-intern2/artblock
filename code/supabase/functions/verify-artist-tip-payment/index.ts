...
Deno.serve(async (request) => {
  ...
  const { data: tipRecord, error: tipLookupError } = await adminClient
    .from("artist_tips")
    .select("id, sender_id, recipient_id, status")
    .eq("razorpay_order_id", orderId)
    .maybeSingle();

  if (tipLookupError || !tipRecord) {
    return jsonResponse({ error: "Tip order was not found." }, 404);
  }

  if (tipRecord.sender_id !== user.id) {
    return jsonResponse({ error: "This tip order does not belong to the current user." }, 403);
  }

  const paidAt = new Date().toISOString();

  const { error: tipUpdateError } = await adminClient
    .from("artist_tips")
    .update({
  ...
  if (tipUpdateError) {
    return jsonResponse({ error: tipUpdateError.message }, 500);
  }
